...
class KuaishouCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            if self.config.ENABLE_CDP_MODE:
                ...
                utils.logger.info(
                    "[KuaishouCrawler] Launching browser using standard mode"
                )
                # 启动浏览器上下文
    ...
    async def search(self, keywords: List[str]):
        ...
        self.logger.info(
            f"开始搜索快手关键词，配置: start_page={start_page}, end_page={self.config.END_PAGE}, max_notes={self.config.CRAWLER_MAX_NOTES_COUNT}, limit_count={ks_limit_count}"
        )
        # 遍历所有关键词
        for keyword in keywords:
            ...
            while (
                ...
                if not vision_search_photo:
                    self.logger.error(
                        f"快手爬虫搜索关键词{keyword}页面{page}返回数据格式异常，增加页码到{page+1}"
                    )
                    page += 1
                    continue
                ...
                if not feeds:
                    self.logger.warning(
                        f"快手爬虫搜索关键词{keyword}页面{page}未获取到视频数据，增加页码到{page+1}"
                    )
                    page += 1
                    continue
                ...
                for video_detail in feeds:
                    ...
                    if video_id:
                        ...
                        await kuaishou_store.update_kuaishou_video(
                            video_item=video_detail, config=self.config
                        )
                ...
                utils.logger.info(
                    f"[KuaishouCrawler.search] Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds after page {page-1}"
                )
    ...
    async def get_specified_videos(self):
        ...
        """
        utils.logger.info(
            "[KuaishouCrawler.get_specified_videos] Parsing video URLs..."
        )
        ...
        for video_url in config.KS_SPECIFIED_ID_LIST:
            try:
                ...
                utils.logger.info(
                    f"Parsed video ID: {video_info.video_id} from {video_url}"
                )
    ...
    async def get_video_info_task(
        ...
        async with semaphore:
            try:
                ...
                await asyncio.sleep(config.CRAWLER_MAX_SLEEP_SEC)
                utils.logger.info(
                    f"[KuaishouCrawler.get_video_info_task] Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds after fetching video details {video_id}"
                )
    ...
    async def get_comments(self, video_id: str, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            try:
                ...
                await asyncio.sleep(config.CRAWLER_MAX_SLEEP_SEC)
                utils.logger.info(
                    f"[KuaishouCrawler.get_comments] Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds before fetching comments for video {video_id}"
                )
    ...
    async def get_creators_and_videos(self) -> None:
        ...
        for creator_url in config.KS_CREATOR_ID_LIST:
            try:
                ...
                utils.logger.info(
                    f"[KuaiShouCrawler.get_creators_and_videos] Parse creator URL info: {creator_info}"
                )
                user_id = creator_info.user_id

                # 从网页内容获取创作者详细信息
                createor_info: Dict = await self.ks_client.get_creator_info(
                    user_id=user_id
                )
                if createor_info:
                ...
                utils.logger.error(
                    f"[KuaiShouCrawler.get_creators_and_videos] Failed to parse creator URL: {e}"
                )
                continue
