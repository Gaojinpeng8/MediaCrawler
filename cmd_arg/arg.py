...
import config
from tools.utils import str2bool, module2object
...
async def parse_cmd():
    ...
    parser.add_argument('--platform', type=str, help='Media platform select (xhs | ks | wb | zh | douyin)', # 平台选择
                        choices=["xhs", "ks", "wb", "zhihu", "dy"], default=config.PLATFORM)
    ...
    parser.add_argument('--init_db',
                        help='Initialize database table structure (sqlite | mysql | postgres)', default=None)
    ...
    config.init_db = args.init_db
    return module2object(config) # 将config换成了一个单独的一个类