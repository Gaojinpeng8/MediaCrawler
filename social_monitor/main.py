...
import cmd_arg
import config
from schema import MonitorPlanEntity, SearchKeywordsEntity
...
# 导入搜索工具
from social_monitor.mcp_client import BAIDUSearch, KIMISearch

# 导入进度缓存
from social_monitor.src.progress_cache import KeywordProgressCache

# 导入工具函数
from social_monitor.utils import datetime_to_bigint  # 日期时间转换为大数
from social_monitor.utils import judge_platform_contain_relation  # 判断平台包含关系
from social_monitor.utils import update_keyword_search_interval  # 更新关键词搜索间隔
from social_monitor.utils import within_hours  # 判断时间是否在指定小时数内

async def _search_keywords_for_plan(
    monitor_plan: MonitorPlanEntity,
    search_tools: list,
    time_range: str,
    nums: int,
    sem: asyncio.Semaphore,
) -> tuple[int, list[str]]:
    """为单个监控方案搜索关键词（同步搜索调用包装为异步）"""
    async with sem:
        keywords = []
        for search_tool in search_tools:
            # search_init_monitor 是同步方法（调用 LLM API），用 to_thread 包装避免阻塞
            kw = await asyncio.to_thread(
                search_tool.search_init_monitor, monitor_plan, time_range, nums
            )
            keywords.extend([w for w in kw if len(w.strip()) > 0])
        return monitor_plan.plan_id, keywords


async def get_keywords(
    monitor_plans: list[MonitorPlanEntity],
    time_range="最近三个月的",
    nums=10,
    monitor_time_interval=24,
    check_node=None,
    MAX_KEYWORD_FETCH_CONCURRENCY=3,
) -> dict:
    """
    获取监控方案对应的关键词
    ...
    # 第一遍遍历：检查缓存，收集需要搜索的方案
    plans_to_search = []
    with tqdm(monitor_plans, desc="检查缓存", unit="方案") as pbar:
        for monitor_plan in pbar:
            pbar.set_description(f"检查方案: {monitor_plan.plan_name[:20]}...")

            search_keywords_entity = await search_keywords_db.get_by_plan_id(
                monitor_plan.plan_id
            )

            if search_keywords_entity:
                check_time = (
                    search_keywords_entity.create_time
                    if check_node == "create"
            ...
            plans_to_search.append(monitor_plan)

    # 第二遍：并发搜索需要获取关键词的方案
    if plans_to_search:
        sem = asyncio.Semaphore(MAX_KEYWORD_FETCH_CONCURRENCY)

        with tqdm(plans_to_search, desc="并发搜索关键词", unit="方案") as pbar:
            async def search_with_progress(plan):
                plan_id, keywords = await _search_keywords_for_plan(
                    plan, search_tools, time_range, nums, sem
                )
                pbar.update(1)
                pbar.set_description(f"搜索完成: {plan.plan_name[:20]}...")
                return plan_id, keywords

            results = await asyncio.gather(*[search_with_progress(p) for p in plans_to_search])

        # 收集结果并写DB
        for plan_id, keywords in results:
            if len(keywords) == 0:
                continue

            search_keyword_id = await search_keywords_db.insert(
                SearchKeywordsEntity(
                    create_time=datetime_to_bigint(datetime.datetime.now()),
                    plan_id=plan_id,
                    keywords=config.KEYWORD_SEPARATOR.join(keywords),
                )
            )
            search_task_list[plan_id] = keywords + [search_keyword_id]
...
async def init_monitor_plan(config_obj, num_keywords=15, max_crawler_notes_count=15):
    ...
    search_task_list = await get_keywords(
        monitor_plans, "最近三个月", num_keywords, 144, "create"
    )
    ...
    for monitor_id, search_keywords in search_task_list.items():
        ...
        # 过滤已完成的关键词，避免重复爬取
        keywords_to_run = [
            kw for kw in search_keywords[:-1] if not cache.is_done(monitor_id, kw)
        ]
        ...
        await monitor_plan_db.update_last_update_time(
            monitor_id, datetime_to_bigint(datetime.datetime.now())
        )

    print("========初始化的方案插入完成了=======")


async def update_monitor_plan(
    config_obj, number_keywords=5, max_crawler_notes_count=15
):
    """
    更新已经初始化的监控方案
    ...
    for monitor_plan in full_monitor_plans:
        ...
        if not within_hours(
            monitor_plan.last_update_time,
            current_time,
            monitor_plan.monitor_time_interval,
        ):
    ...
    for mp in monitor_plans:
        res = await get_keywords(
            [mp], "最近几天", number_keywords, mp.monitor_time_interval, "update"
        )
...
async def update_search_keywords(config_obj, max_crawler_notes_count=10):
    ...
    for search_keyword in search_keywords_list:
        ...
        if not within_hours(
            search_keyword.last_update_time,
            current_time,
            search_keyword.update_interval,
        ):
    ...
    for monitor_id, search_keywords in search_task_list.items():
        ...
        keywords_to_run = [
            kw for kw in search_keywords[:-2] if not cache.is_done(monitor_id, kw)
        ]
        ...
        for keyword in keywords_to_run:
            ...
            for platform, crawler in crawlers.crawlers.items():
                ...
                if judge_platform_contain_relation(platform, tmp_monitor.platform):
                    ...
                    crawler.config.ENABLE_GET_SUB_COMMENTS = False  # 不获取子评论
                    crawler.config.SEARCH_KEYWORD_ID = (
                        f"{search_keywords[-2]}-{keyword}"  # 组合搜索关键词ID
                    )
                    crawler.config.CRAWLER_MAX_NOTES_COUNT = (
                        max_crawler_notes_count  # 最大爬取数量
                    )
...
async def check():
    ...
    await monitor_plan_db.update_last_update_time(
        779, datetime_to_bigint(datetime.datetime.now())  # 监控方案ID  # 当前时间
    )
