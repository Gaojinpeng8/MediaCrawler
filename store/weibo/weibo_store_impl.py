...
class WeiboDbStoreImplement(AbstractStore):
    ...
    async def store_creator(self, creator: Dict):
        ...
        说明：当前 schema/tables.sql 未定义创作者表，保留占位实现以满足抽象接口，避免实例化错误。
        """
        return None
    async def store_hotlist(self, hotlist_item: Dict):
        """
    ...
    async def store_content(self, content_item: Dict):
        ...
        note_detail = await query_content_entity_by_content_id(content_id=note_id)
        if not note_detail:
    ...
    async def store_comment(self, comment_item: Dict):
        ...
        comment_detail = await query_comment_entity_by_comment_id(comment_id=comment_id)
    ...
    async def store_extract_info(self, extract_info_item: Dict):
        ...
        extract_info_detail = await query_extract_info_entity(content_id, content_type, content_source)
    ...
    async def store_pic(self, pic_item):
        """改为不使用 MongoDB，直接返回图片 URL 的 JSON 字符串
        Args:
            pic_item: {"pic_urls": [url1, url2, ...]}

        Returns:
            str | None: JSON 字符串（存入 MySQL 的 content_pics 字段），无图片则返回 None
        """
        pic_urls = pic_item.get("pic_urls")
        if not pic_urls:
            return None
        try:
...
class WeiboJsonStoreImplement(AbstractStore):
    ...
    async def store_hotlist(self, hotlist_item: Dict):
        """Weibo hotlist JSON storage implementation"""
        await self.save_data_to_json(hotlist_item, "title")

    async def store_extract_info(self, extract_info_item: Dict):
        """Weibo extract_info JSON storage implementation"""
        await self.save_data_to_json(extract_info_item, "extract_info")

    async def store_pic(self, pic_item):
        """Weibo picture placeholder for JSON mode (no binary store)"""
        # 将图片 URL 列表也以 JSON 记录，便于后续处理
        await self.save_data_to_json({"pics": pic_item.get("pic_urls", [])}, "pics")
        # 返回一个占位ID或计数，当前场景返回长度
        return len(pic_item.get("pic_urls", []))

    async def store_video(self, video_item):
        """Weibo video placeholder for JSON mode (no binary store)"""
        await self.save_data_to_json(video_item, "videos")

    async def store_creator(self, creator: Dict):
        """Weibo creator JSON storage implementation"""
