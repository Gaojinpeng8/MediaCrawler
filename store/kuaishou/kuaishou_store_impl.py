...
class KuaishouCsvStoreImplement(AbstractStore):
    ...
    async def store_creator(self, creator: Dict):
        """
        Kuaishou creator CSV storage implementation
        Args:
...
class KuaishouDbStoreImplement(AbstractStore):
    ...
    async def store_content(self, content_item: Dict):
        ...
        video_detail = await query_content_entity_by_content_id(content_id=content_id)
        if not video_detail:
    ...
    async def store_creator(self, creator: Dict):
        """
        Kuaishou creator DB storage implementation (no-op)
        Args:
            creator: creator item dict
        ...
        Returns:
            None
        """
        # 数据库 schema 中未定义独立的创作者表，此处占位实现以满足抽象类要求
        return None
